...
<body class="bg-[#2D2D2D] min-h-screen">
    ...
    <div class="min-h-screen w-full bg-[#2D2D2D]">
        ...
        <div class="max-w-lg mx-auto">
            ...
            $shopItems = [
                ...
                [
                    ...
                    'price' => 72,
                    'stock' => 18,
                    'slug' => 'seasonal-hoodie-winter-2026',
                    'images' => [
            ...
            <div class="gallery-grid">
                ...
                <div class="gallery-item group cursor-pointer" onclick="openPostView({{ $index }})">
                    ...
                    @if($item['stock'])
                    <div class="stock-badge">{{ $item['stock'] }} left</div>
                    @endif
        ...
        <div id="post-scroll-view" class="fixed inset-0 bg-[#2D2D2D] z-50 hidden" style="overflow: hidden;">
            ...
            <div class="absolute top-0 left-0 right-0 bg-gradient-to-b from-black/60 to-transparent z-10 pointer-events-none" style="height: 80px;">
                <div class="px-4 py-3 flex items-center pointer-events-auto">
                    ...
                    <span class="flex-1 text-center text-white font-semibold font-['Bricolage_Grotesque',sans-serif]">Shop</span>
                    <div class="w-6"></div>
                </div>
            </div>
            ...
            <div id="posts-container" style="height: 100%; overflow-y: auto; scroll-snap-type: y mandatory; -webkit-overflow-scrolling: touch;">
                @foreach($shopItems as $index => $item)
                <div class="post-item" data-index="{{ $index }}" data-images="{{ json_encode($item['images']) }}" data-slug="{{ $item['slug'] }}" style="height: 100vh; scroll-snap-align: start; scroll-snap-stop: always; display: flex; flex-direction: column; position: relative;">
                    <!-- Media Carousel -->
                    ...
                    <div style="position: absolute; bottom: 0; left: 0; right: 0; padding: 24px; padding-bottom: 40px; z-index: 5;">
                        ...
                        <a href="/shop/{{ $item['slug'] }}" class="block cursor-pointer hover:opacity-80 transition-opacity">
                            <div class="flex items-end justify-between">
                                <div>
                                    <p class="text-white text-lg font-semibold font-['Bricolage_Grotesque',sans-serif]">{{ $item['title'] }}</p>
                                    <div class="flex items-center gap-3 mt-1">
                                        <p class="text-gray-300 text-sm font-['Bricolage_Grotesque',sans-serif]">{{ $item['category'] }}</p>
                                        <span class="text-[#FDFBD8] text-lg font-bold font-['Bricolage_Grotesque',sans-serif]">${{ $item['price'] }}</span>
                                    </div>
                                    @if($item['stock'])
                                    <p class="text-[#FF5349] text-xs font-['Bricolage_Grotesque',sans-serif] mt-1">Only {{ $item['stock'] }} left in stock</p>
                                    @endif
                                </div>
        ...
        <x-reviews-carousel category="shop" />
    ...
    <script>
        ...
        function openPostView(index) {
            // Desktop: redirect to product detail page
            if (window.innerWidth > 768) {
                const postItem = document.querySelector(`.post-item[data-index="${index}"]`);
                if (postItem && postItem.dataset.slug) {
                    window.location.href = '/shop/' + postItem.dataset.slug;
                }
                return;
            }
